...
risk_mortality_summary2 <- data.frame(
  Month = rep(1:12, 2),
  Mortality_Rate = runif(24, 0.01, 0.1),
  CVD = rep(c("Yes", "No"), each = 12),
  TRTMT = rep(c("Placebo", "Treatment"), each = 12)
)
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    sidebarMenu(
      ...
      sliderInput("ageFilter", "Age", min = min(dig.df$AGE, na.rm = TRUE), 
                  max = max(dig.df$AGE, na.rm = TRUE), value = range(dig.df$AGE, na.rm = TRUE)),
      sliderInput("bmiFilter", "BMI", min = min(dig.df$BMI, na.rm = TRUE), 
                  max = max(dig.df$BMI, na.rm = TRUE), value = range(dig.df$BMI, na.rm = TRUE)),
      sliderInput("sysbpFilter", "Systolic Blood Pressure", min = min(dig.df$SYSBP, na.rm = TRUE), 
                  max = max(dig.df$SYSBP, na.rm = TRUE), value = range(dig.df$SYSBP, na.rm = TRUE)),
      sliderInput("diabpFilter", "Diastolic Blood Pressure", min = min(dig.df$DIABP, na.rm = TRUE), 
...
server <- function(input, output) {
  ...
  filtered_data <- reactive({
    dig.df %>%
      filter(
        ...
        BMI >= input$bmiFilter[1], BMI <= input$bmiFilter[2],
        SYSBP >= input$sysbpFilter[1], SYSBP <= input$sysbpFilter[2],
        DIABP >= input$diabpFilter[1], DIABP <= input$diabpFilter[2]
      )
  })
  
  output$hospital_plot <- renderPlotly({
    hospitalizations <- ggplot(filtered_data(), aes(x = HOSP, fill = TRTMT)) +
  ...
  output$cvd_mortality_plot <- renderPlotly({
    CVD_mortality <- ggplot(filtered_data()) +
      geom_mosaic(aes(x = product(CVD), fill = DEATH)) +
      labs(title = "CVD and Mortality by Treatment", x = "Suffers from CVD", fill = "Mortality") +
      facet_wrap(~TRTMT) +
  ...
  output$whf_hosp_plot <- renderPlotly({
    WHF_HOSP <- ggplot(filtered_data(), aes(x = HOSP, fill = WHF)) +
      geom_bar() +
      facet_wrap(~TRTMT) +
      scale_fill_manual(values = c("Yes" = "#90EE90", "No" = "#607B6B")) +
      labs(title = "Hospitalizations for WHF by Treatment", x = "Hospitalized?", fill = "WHF")
    ggplotly(WHF_HOSP)
  })
  ...
  output$mortality_rate_plot <- renderPlotly({
    plot2 <- ggplot(risk_mortality_summary2, aes(x = Month, y = Mortality_Rate, color = CVD, linetype = CVD)) +
      ...
      labs(x = "Time in Months", y = "Mortality Rate",
           title = "Effect of CVD on Mortality Over Time by Treatment",
           color = "CVD", linetype = "CVD")
    ggplotly(plot2)
  })
  
  output$bp_relationship_plot <- renderPlotly({
    plot3 <- ggplot(filtered_data(), aes(x = SYSBP, y = DIABP, colour = TRTMT, shape = TRTMT)) +
      geom_point(size = 1.2) +
      geom_smooth(method = "lm", formula = y ~ x, se = FALSE) +
      labs(title = "Relationship between Systolic and Diastolic BP",
           x = "Systolic BP (mmHg)", y = "Diastolic BP (mmHg)",
